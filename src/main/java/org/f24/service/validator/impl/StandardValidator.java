...
public class StandardValidator extends FormValidator {
    ...
    private void validateSections() throws ResourceException {
        ...
        List<? extends Record> imuRecordList = this.form.getImuSection().getImuRecordList();
        validateDebitandCredit(imuRecordList);

        List<? extends Record> inailRecordList = this.form.getSecuritySection().getInailRecords();
        validateDebitandCredit(inailRecordList);

        List<? extends Record> socSecurityRecordList = this.form.getSecuritySection().getSocialSecurityRecordList();
    ...
    private void validateDebitandCredit(List<? extends Record> targetRecordList) throws ResourceException {
        if (targetRecordList != null) {
            for (Record recordItem : targetRecordList) {
                if (recordItem.getDebitAmount() != null && recordItem.getDebitAmount() != "0"
                        && recordItem.getCreditAmount() != null && recordItem.getCreditAmount() != "0") {
