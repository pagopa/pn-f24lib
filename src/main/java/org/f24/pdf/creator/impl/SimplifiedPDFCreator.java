...
public class SimplifiedPDFCreator extends PDFFormManager implements PDFCreator {
    ...
    private void setHeader() throws Exception {
        ...
        if(header != null) {
            setField("attorney", header.getAttorneyTo());
    ...
    private void setPersonalData() throws Exception {
        PersonalData personalData = this.form.getContributor().getPersonalData();
        if(personalData != null) {
            setField("corporateName", personalData.getCorporateName());
            setField("name", personalData.getName());
            setField("dateOfBirth", new SimpleDateFormat("ddMMyyyy").format(personalData.getDateOfBirth()));
            setField("sex", personalData.getSex() == 1 ? "M" : "F");
    ...
    private void setContributor() throws Exception {
        ...
        if(contributor != null) {
            ...
            setField("deedCode", contributor.getDeedCode());
    ...
    private void setPaymentMotiveSection(int copyIndex) {
        try {
            ...
            for (int index = 1; index <= paymentMotiveRecordList.size(); index++) {
                ...
                if (record.getBuilding() != null) setField("building" + index, "X");
    ...
    private void setPaymentDetails() throws Exception {
        ...
        if(paymentDetails != null) {
            setField("dateOfPayment", new SimpleDateFormat("ddMMyyyy").format(paymentDetails.getDateOfPayment()));
            setField("company", paymentDetails.getCompany());
            setField("cabCode", paymentDetails.getBank());
            setField("checkNumber", paymentDetails.getCheckNumber());
            setField("abiCode", paymentDetails.getAbiCode());
            setField("isBank", paymentDetails.getBank() != null ? "X" : "");
            setField("isCircular", paymentDetails.getCircular() != null ? "X" : "");
